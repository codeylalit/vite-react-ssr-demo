...
interface SimpleTranscriptViewProps {
  segments: TranscriptSegment[];
  isRecording: boolean;
  onClear: () => void;
}

export const SimpleTranscriptView: React.FC<SimpleTranscriptViewProps> = ({
  segments,
  isRecording,
  onClear,
}) => {
        ...
        {segments.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full min-h-[300px] text-gray-500">
            ...
            <p className="text-sm text-center">
              Tap the microphone to begin real-time speech recognition with ShunyaLabs Realtime Service
            </p>
          </div>
        )}
          ...
          <div className="space-y-1">
            {segments.map(segment => (
              <span
                ...
                title={`${segment.isFinal ? 'Final' : 'Partial'} - Confidence: ${(segment.confidence * 100).toFixed(1)}%`}
              >
                {segment.text}
              </span>
            ))}
